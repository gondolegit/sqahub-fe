...
const OAuth2RedirectPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!token || !userId || !username || !role) {
            setError('Respons Google tidak lengkap. Silakan coba lagi.');
            return;
        }

        applySession({ token, userId, username, role: role as UserRole });
        navigate('/dashboard', { replace: true });
        // Hanya dijalankan sekali saat mount — searchParams/navigate/applySession stabil untuk siklus hidup halaman ini.
