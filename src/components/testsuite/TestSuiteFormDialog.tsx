...
const DetailItem = ({ icon, label, value, isPre, className }: { icon: React.ReactNode, label: string, value: string | null | undefined, isPre?: boolean, className?: string }) => (
    <div className={`space-y-1.5 ${className}`}>
        <div className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-wider">{icon} {label}</div>
        <div className={`text-xs text-muted-foreground leading-relaxed ${isPre ? 'font-mono bg-muted p-2 rounded whitespace-pre-wrap border border-border' : ''}`}>
            {value || '-'}
        </div>
    </div>
);
...
const TagBadge = ({ tags }: { tags: string }) => {
    return (
        <div className="flex flex-wrap gap-1 mt-2">
            {tags.split(',').map((t, i) => (
                <span key={i} className="text-[10px] px-2 py-0.5 bg-muted text-muted-foreground rounded-full border border-border font-medium">
                    #{t.trim()}
                </span>
            ))}
        </div>
                ...
                <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                            <Badge variant="outline" className="bg-muted text-muted-foreground font-bold">TC-{field.idTestCase}</Badge>
                            <Badge className="bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 border-none uppercase text-[9px]">{tcFullData?.type || 'FUNCTIONAL'}</Badge>
                        </div>
                        <h4 className="font-bold text-base md:text-lg text-foreground tracking-tight truncate">{field.testCaseName}</h4>

                        {/* PERBAIKAN: Menampilkan Tag dari Response */}
                        {tcFullData?.tag && <TagBadge tags={tcFullData.tag} />}

                        <p className="text-xs text-muted-foreground mt-2 line-clamp-1 italic">{tcFullData?.description}</p>
                    </div>
                    ...
                    <div className="flex gap-2 shrink-0">
                        <Popover>
                            ...
                            <PopoverContent
                                className="w-[280px] sm:w-[350px] md:w-[450px] p-0 shadow-2xl border-border overflow-hidden flex flex-col"
                                side="bottom"
                                align="end"
                                sideOffset={10}
                            >
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-[100vw] w-full h-[100dvh] md:h-[95vh] md:max-w-[98vw] p-0 flex flex-col overflow-hidden">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden bg-muted/50">

                        {/* HEADER */}
                        <header className="flex items-center justify-between p-4 border-b bg-card shrink-0 z-30 shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        <span className="text-[10px] text-muted-foreground font-medium">ID: #{initialProjectId}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-1 overflow-hidden">
                            ...
                            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                                {/* SEARCH & PICKER */}
                                <div className="p-4 bg-card border-b shadow-sm z-10">
                                    <div className="max-w-5xl mx-auto space-y-3">
                                        ...
                                        <ScrollArea className="h-[120px] md:h-[160px] rounded-xl border border-border bg-muted/30 p-2">
                                                    ...
                                                    filteredTC.map((tc: TestCase) => {
                                                        ...
                                                        return (
                                                            <div key={tc.id} onClick={() => toggleTC(tc)}
                                                                className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer select-none ${isSelected ? 'bg-primary border-primary text-white shadow-lg' : 'bg-card border-border hover:bg-muted'}`}>
                                                                <CheckSquare className={`w-4 h-4 ${isSelected ? 'text-white' : 'text-slate-300'}`} />
                                                                <div className="min-w-0 flex-1">
                                                                    <p className={`text-[10px] font-bold ${isSelected ? 'text-white/70' : 'text-muted-foreground'}`}>TC-{tc.id}</p>
